...
function renderWarning(){var n=new Array,e=-1;return n[++e]='<p id="w-browser-warning" class="w-error" style="margin:0;">',n[++e]="You are using an old browser which is unsupported. Some features may not work correctly. ",n[++e]='Consider <a href="http://www.whatbrowser.org/intl/en_uk/">upgrading</a> to a modern browser. ',n[++e]='<span id="w-dismiss-browser-warning" class="w-false-link">Dismiss.</span> ',n[++e]="</p>",n.join("")}function oldBrowserWarning(){if(document.all&&!document.addEventListener){var n=document.createElement("div");n.innerHTML=renderWarning(),document.body.insertBefore(n,document.body.childNodes[0])}document.getElementById("w-dismiss-browser-warning")&&(document.getElementById("w-dismiss-browser-warning").onclick=function(){var n=document.getElementById("w-browser-warning");n.parentNode.removeChild(n)})}function svgInject(){this.inject=function(n){n="undefined"!=typeof n?n:"",$(n+" .w-icon").each(function(){var n=$(this).data("name");$(this).html(w_icons[n])})}}window.attachEvent&&window.attachEvent("onload",oldBrowserWarning);var wModal,datePickerOptions={show_icon:!1},RecaptchaOptions={theme:"custom",custom_theme_widget:"recaptcha_widget"};$(document).ready(function(){$(".w-breadcrumb li:last-child a").click(function(n){n.preventDefault()}),$(".w-file-input button, .w-file-input span").on("click",function(n){n.preventDefault(),$(this).siblings("input[type=file]").trigger("click")}),$("input[type=file]").change(function(){var n=$(this).val().replace(/\\/g,"/");n=n.split("/").pop(),$(this).siblings("span").text(n)})});
