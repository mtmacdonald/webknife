function oldBrowserWarning(){if(document.all&&!document.addEventListener){var e=new Array,i=-1;e[++i]='<p id="w-browser-warning" class="w-error" style="margin:0;">',e[++i]="You are using an old version of Internet Explorer which is unsupported. Some features may not work correctly. ",e[++i]='Consider <a href="http://www.whatbrowser.org/intl/en_uk/">upgrading</a> to a modern browser. ',e[++i]='<span id="w-dismiss-browser-warning" class="w-false-link">Dismiss.</span> ',e[++i]="</p>",$("body").prepend(e.join(""))}}var wModal,datePickerOptions={show_icon:!1},RecaptchaOptions={theme:"custom",custom_theme_widget:"recaptcha_widget"};$(document).ready(function(){oldBrowserWarning(),$("body").on("click","#w-dismiss-browser-warning",function(){$("#w-browser-warning").remove()}),$(".w-breadcrumb li:last-child a").click(function(e){e.preventDefault()}),$(".w-file-input-button").click(function(){$(this).prev("input[type=text]").prev("input[type=file]").trigger("click")}),$(".w-file-input").click(function(){$(this).prev("input[type=file]").trigger("click")}),$("input[type=file]").change(function(){var e=$(this).val().replace(/\\/g,"/");e=e.split("/").pop(),$(this).next("input[type=text]").val(e)})});
